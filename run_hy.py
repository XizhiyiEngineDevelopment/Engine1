...
def update_input_dict(answer, input_dict):
    return input_dict
...
def set_prompt(input_variables):
    allowed_tools = input_variables["allowed_tools"]
    assert isinstance(allowed_tools, list) 
    assert "status" in input_variables.keys()

    # input_variables["status"]: ["input", "search", "calculator", "finished"] 
    if input_variables["status"] == "input":
        process_input()
    elif input_variables["status"] == "search":
        if "search" in allowed_tools:
            ...
            raise Exception("No searching tools provided!")
    elif input_variables["status"] == "calculator":
        if "search" in allowed_tools:
            process_calculator()
        else:
            raise Exception("No calculator tools provided!")
    elif input_variables["status"] == "finished":
        process_finished()
    else:
        raise ValueError(f"Input variables {input_variables} not recognized")
...
def run(input_dict):
    ...
    for i in range(max_iteration):
        prompt = set_prompt(input_dict) # 把输入填充到对应模板中
        answer = get_completion(prompt=prompt)
        if "\nFinal Answer: " in answer:
            return answer
        else: #处理返回的answer，更新input_dict
            input_dict = update_input_dict(answer, input_dict)

    return "Max iteration, Overtime!"
